...
def main():
    ...
    def stop_session():
        ...
        def _debrief():
            ...
            # --- Send by email (summary only, no transcript) ---
            recipient = os.environ.get("GMAIL_USER", "")
            subject_contact = f" — {contact_email}" if contact_email else ""
            email_subject = f"Sales call debrief{subject_contact} ({duration_min} min)"
            ok, msg = send_debrief_email(recipient, email_subject, summary_md)
            if ok:
                tip_queue.put({"type": "tip", "text": f"✉️ Email {msg}"})
            else:
                tip_queue.put({"type": "tip", "text": f"WARN: Email failed — {msg}"})
